#!/usr/bin/env bash
# cursor-cloud-setup: activate ubuntu@cursor-cloud, or init .cursor templates.
# Store paths come from the Nix wrapper via TEMPLATE_JSON, TEMPLATE_SH, JQ.
set -euo pipefail

template_json="${TEMPLATE_JSON:?TEMPLATE_JSON is required}"
template_sh="${TEMPLATE_SH:?TEMPLATE_SH is required}"
jq="${JQ:?JQ is required}"
