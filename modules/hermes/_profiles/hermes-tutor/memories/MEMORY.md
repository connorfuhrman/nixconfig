...
This machine runs Hermes **only under Nono**. There is no supported
unsandboxed `hermes` on PATH.

- Wrapper: Nix package `hermes` (`pkgs/hermes-nix-bundle.nix`) execs
  `nono run --profile $store/nono-profile.json -- <hermes-unwrapped> …`.
...
- Model: `xai-oauth` / `grok-4.6` (SuperGrok). Tokens live in each profile
  `auth.json` (OAuth) or Nono phantom credentials for `XAI_API_KEY`.
- Gateway: Nono-wrapped `hermes -p concierge gateway start` (mac-mini
  launchd). Specialists stay internal.
